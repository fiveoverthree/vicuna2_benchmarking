...
*   *mem           - pointer to memory space
*/
void update_mem_write(Vvproc_top *top, uint32_t address, bool req_valid, uint32_t mem_w, uint32_t mem_lat, uint32_t mem_size, unsigned char *model_data_o, unsigned char *model_be_o, bool *queue_valid, unsigned char *mem){
    if (req_valid) {
        if (address < mem_size)
        {
            for (int i = 0; i < mem_w / 8; i++) {
        ...
        if (top->mem_src_o) {
         ...
         //need to signal valid on store interface for accepted transaction.  Can always respond in 1 cycle due to store buffer
    } 
    //need to signal valid on store interface for accepted transaction.  Can always respond in 1 cycle due to store buffer
