...
#(
  ...
  cva6_pipeline #(
      ...
      .clk_i(clk_i),      
      .rst_ni(rst_ni),    
      .boot_addr_i('h000022d4),//TODO:  CURRENTLY SET TO 80 to start at the reset vector -> could set to x8000080 to match with spike?
...
always_comb begin
  ...
  obi_load_rsp.gnt = (!obi_store_req.req && !vcore_xif.mem_valid && no_outstanding_vlsu) && mem_gnt_i; //only grant cv32a60x the memory interface when no vector loads or stores are outstanding
  obi_store_rsp.gnt = (!vcore_xif.mem_valid && no_outstanding_vlsu) && mem_gnt_i; //only grant cv32a60x the memory interface when no vector loads or stores are outstanding
