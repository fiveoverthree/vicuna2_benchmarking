...
public:
  ...
  inline void begin_measurement() { address_match_start(); }
  inline void end_measurement() { address_match_end(); }

  void terminate(int code) {
    sh_exit_args args = {ADP_Stopped_ApplicationExit,
                         static_cast<uint32_t>(code)};
    riscv_semihosting_exit_extended(static_cast<int>(SH_SYS_EXIT),
                                    (void *)&args);
  }

  static inline long riscv_semihosting_exit_extended(int reason,
                                                     void *argpack) {
